...
def extract_cartesian_g09(filename):
...
if __name__ == '__main__':
    ...
    # Get all subdirectories (no recursion)
    listdir = [item for item in os.listdir('./') if os.path.isdir(item)]

    for idir in listdir:
        newfile = open(idir+".xyz",'w')
        print(idir)
        for path in Path(idir).rglob("*.log"):
            c_log = str(path.parent)+"/"+path.name
            coordlist = extract_cartesian_g09(c_log)
            natoms = coordlist.count('\n')
            name = c_log
            newfile.writelines(str(natoms)+"\n")
            newfile.writelines(name+"\n")
            newfile.write(coordlist)
